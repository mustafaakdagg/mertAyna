...
void loop() {
  ...
  if (lt < 250 && rt < 250 && ld < 250 && rd < 250) {      //eğer karanlığa girdiyse
    karanlikKontrolu();
    while (lt < 250 && rt < 250 && ld < 250 && rd < 250) {
      ldrRead();
    }
  }
  else {                                                  //normal çalışma
    ...
    ldrRead();
    gunesBul();
    if (avt-avd <= 15 && avd-avt <= 15 && avl-avr <= 15 && avr-avl <= 15 ) {
      yansit();
      timer = 0;
...
void gunesBul() {
  ...
  if (lt > 250 || rt > 250 || ld > 250 || rd > 250) {
    ...
    if (dvert > tol || dvert < (-tol)) {// check if the diffirence is in the
      if (avt > avd) {
        servov = ++servov;
        if (servov >= servovLimitHigh) {
    ...
    if (dhoriz > tol || dhoriz > (-tol)) {// check if the diffirence is in the {
      if (avl > avr ) {
        servoh = --servoh;
